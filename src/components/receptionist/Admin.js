...
import { Spinner, Text, Button, AlertDialog, Callout } from '@radix-ui/themes';
    ...
    <div style={{ width: '100%' }}>
      ...
      <Row style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', marginLeft: 0, marginRight: 0, marginTop: 20 }}>
        <Col xs={12} sm={12} md={12} lg={9} xl={7} style={{ padding: 0 }}>
          <Callout.Root color="red" style={{ padding: 20 }}>
            <div>
              <Text size="2" weight="bold">Delete receptionist</Text>
              <Text size="1" as='div' style={{ marginTop: 0 }}>Permanently delete your receptionist.</Text>
              <AlertDialog.Root>
                <AlertDialog.Trigger>
                  <Button variant="surface" color='red' style={{ marginTop: 20 }} loading={deleteLoading}><Trash size={16} /> {deleteLoading ? 'Deleting...' : 'Delete'}</Button>
                </AlertDialog.Trigger>
                <AlertDialog.Content>
